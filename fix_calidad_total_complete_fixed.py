...
def fix_calidad_total_complete():
    ...
    if current:
        ...
        if complete_count == len(required_sections):
            ...
            print(
                f"⚠️ Content incomplete - missing {len(required_sections) - complete_count} sections"
            )
    else:
    ...
    total_length = sum(
        len(str(complete_content[section])) for section in complete_content
    )
    ...
    if success:
        ...
        if verified:
            final_complete = sum(
                1
                for section in required_sections
                if verified.get(section) and len(str(verified.get(section, ""))) > 50
            )
            print(
