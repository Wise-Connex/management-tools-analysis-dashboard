...
class KeyFindingsService:
    ...
    def __init__(
        ...
        # Initialize Key Findings database - SIMPLIFIED: Remove caching layer
        # Database path will be set later if needed for model performance logging
        self.kf_db_manager = None
    ...
    async def generate_key_findings(
        ...
        try:
            ...
            logging.info(
                f"🔧 Multi-source AI-generated values - Model: {content.get('model_used', 'N/A')}, Data points: {content.get('data_points_analyzed', 'N/A')}"
            )
    ...
    async def _generate_single_source_analysis(
        ...
        try:
            ...
            logging.info(
                f"🔍 hasattr(self, 'single_source_fixer'): {hasattr(self, 'single_source_fixer')}"
            )
            if hasattr(self, "single_source_fixer"):
                logging.info(
                    f"🔍 self.single_source_fixer type: {type(self.single_source_fixer)}"
                )
            else:
            ...
            logging.info(
                "🔧 Single source fixer disabled - using improved section extraction"
            )
            content = content  # Use content as-is
            is_valid = True
            if not is_valid:
                logging.warning(
                    "⚠️ Single source structure validation failed, but continuing"
                )
            ...
            logging.info(
                f"🔧 AI-generated values - Model: {content.get('model_used', 'N/A')}, Data points: {content.get('data_points_analyzed', 'N/A')}"
            )
