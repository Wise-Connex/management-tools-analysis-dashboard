...
# Import precomputed findings database (Phase 4: Database-First Strategy)
import sys
from pathlib import Path

# Initialize precomputed database manager (None if import fails)
_precomputed_db_manager = None
try:
    # Add tools-dashboard root directory to path
    ...
    print("✅ Precomputed findings database loaded for Phase 4 integration")
except ImportError as e:
    print(f"Warning: Could not import precomputed findings database: {e}")
    _precomputed_db_manager = None
...
class KeyFindingsService:
    ...
    def __init__(
        ...
        # Initialize database-first service for Phase 4 integration
        self.database_first_service = None
        if _precomputed_db_manager:
            try:
                from .database_first_service import DatabaseFirstService

                self.database_first_service = DatabaseFirstService(self)
                print("✅ Database-first service integrated for instant cached results")
            except ImportError as e:
    ...
    async def generate_key_findings(
        ...
        """
        Generate Key Findings analysis with database-first strategy.

        First checks precomputed database (Phase 3.5 with 1,302 analyses),
        then falls back to live AI generation if needed.
        ...
        # Check precomputed database first for instant results
        if not force_refresh and self.database_first_service:
            try:
                print(
                    f"🔍 DATABASE-FIRST: Checking precomputed database for {tool_name}"
                )
                db_result = self.database_first_service.get_analysis_from_database(
                    tool_name, selected_sources, language
                )
                if db_result:
                    print(
                        f"✅ INSTANT RESULT: Found in precomputed database ({db_result['response_time_ms']}ms)"
                    )
                    return db_result
            except Exception as e:
                print(f"⚠️ Database-first check failed: {e}")
                # Continue with normal flow if database-first fails

        print(
            f"🔄 DATABASE-FIRST: Not in cache or forced refresh, using live AI for {tool_name}"
        )
        # Continue with existing logic for live AI generation
        ...
        try:
            ...
            logging.info(
                f"Cache miss for scenario {scenario_hash[:8]}... Generating new analysis"
            )
    ...
    async def _generate_single_source_analysis(
        ...
        try:
            ...
            ai_result = await self.ai_service.generate_single_source_report(
