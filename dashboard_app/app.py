...
""" + "\n\n"
                        ...
                        # Set global flag to bypass section parsing - we have clean combined content
                        globals()['has_clean_combined_content'] = True
                    ...
                    if principal_findings_content:
                        ...
                        # Check if we have clean combined content that should bypass parsing
                        if globals().get('has_clean_combined_content', False):
                            print("🔍 DEBUG: Using clean combined content - bypassing all section parsing")
                            # Create single section with the complete clean content
                            sections = [principal_findings_content]
                            # Clear the flag for future requests
                            globals()['has_clean_combined_content'] = False
                        else:
                            # First try: Split by double newlines
                            sections = principal_findings_content.split('\n\n')

                        # If we only get 1 section, check if it's clean combined content or needs pattern-based approach
                        if len(sections) <= 1:
                            if globals().get('has_clean_combined_content', False):
                                print("🔍 DEBUG: Single section with clean combined content - using as-is")
                                # Keep sections as is - already contains complete clean content
                            else:
