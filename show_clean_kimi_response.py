#!/usr/bin/env python3
"""
Clean script to display the Kimi K2 response with proper formatting.
Handles the current response structure and formats it correctly.
"""
...
async def show_clean_kimi_response():
    """Display the Kimi K2 response with proper formatting."""

    print("🎯 CLEAN KIMI K2 ANALYSIS - PROPER FORMAT")
    print("=" * 80)
    ...
    try:
        ...
        if isinstance(response, dict) and "content" in response:
            ...
            # Now display each section with proper formatting
            print(f"\n🎯 COMPLETE KIMI K2 ANALYSIS:")
            print("=" * 80)
            ...
            if "executive_summary" in content and content["executive_summary"]:
                ...
                if exec_summary.startswith("```json\n"):
                    exec_summary = exec_summary[8:]  # Remove ```json
                if exec_summary.endswith("\n```"):
                    exec_summary = exec_summary[:-4]  # Remove ```
                try:
                    # Try to parse as JSON to get clean content
                    exec_json = json.loads(exec_summary)
                    if "executive_summary" in exec_json:
                        print(exec_json["executive_summary"])
                    else:
                        print(exec_summary)
                except:
                    print(exec_summary)
                    ...
                    for i, finding in enumerate(findings, 1):
                        if isinstance(finding, dict) and "bullet_point" in finding:
                            ...
                            if bullet_point.startswith("```json\n"):
                                bullet_point = bullet_point[8:]
                            if bullet_point.endswith("\n```"):
                                bullet_point = bullet_point[:-4]

                            print(f"{i}. {bullet_point}")
            ...
            if "pca_analysis" in content and content["pca_analysis"]:
                ...
                if pca_content.startswith("```json\n"):
                    pca_content = pca_content[8:]
                if pca_content.endswith("\n```"):
                    pca_content = pca_content[:-4]
                try:
                    # Try to parse as JSON to get clean content
                    pca_json = json.loads(pca_content)
                    if "executive_summary" in pca_json:
                        print(pca_json["executive_summary"])
                    else:
                        print(pca_content)
                except:
                    print(pca_content)
...
if __name__ == "__main__":
    exit_code = asyncio.run(show_clean_kimi_response())
    sys.exit(exit_code)