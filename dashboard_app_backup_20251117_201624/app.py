...
def clean_content_headers(content, field_key):
    ...
    # Remove horizontal rule separators that confuse markdown parser
    ...
    # FIX: Handle "Implicaciones Gerenciales" section FIRST to protect numbered items
    # Look for this section and convert it to a header, then protect its numbered items
    implications_patterns = [
        r"^Implicaciones Gerenciales$",
        r"^IMPLICACIONES GERENCIALES$",
    ]

    for pattern in implications_patterns:
        implications_match = re.search(pattern, cleaned_content, flags=re.MULTILINE)
        if implications_match:
            ...
            # Now handle the numbered items in this section
            implications_section_match = re.search(
                r"## Implicaciones Gerenciales(.*?)(?=##|\Z)",
                cleaned_content,
                re.DOTALL | re.IGNORECASE,
            )
            if implications_section_match:
                implications_text = implications_section_match.group(1)
                # Convert numbered items to list format
                protected_implications = re.sub(
                    r"^(\d+)\.\s+([^#\n]+?)$",
                    r"- \2",
                    implications_text.strip(),
                    flags=re.MULTILINE,
                )
                cleaned_content = cleaned_content.replace(
                    implications_section_match.group(0),
                    f"## Implicaciones Gerenciales\n\n{protected_implications}",
                )
            break

    # CRITICAL FIX: Convert ALL section headers to proper markdown headers (##)

    # 1. Main sections: "1. Resumen Ejecutivo", "2. Análisis Temporal"
    main_section_pattern = r"^(\d)\.\s+([A-ZÁÉÍÓÚÑ][^:\n]+)$"
    cleaned_content = re.sub(
        main_section_pattern, r"## \1. \2", cleaned_content, flags=re.MULTILINE
    )

    # 2. Multi-digit main sections: "10. Something"
    multi_digit_main_pattern = r"^(\d{2,})\.\s+([A-ZÁÉÍÓÚÑ][^:\n]+)$"
    cleaned_content = re.sub(
        multi_digit_main_pattern, r"## \1. \2", cleaned_content, flags=re.MULTILINE
    )

    # 3. Sub-sections: "2.1 Tendencia", "3.2 Análisis"
    subsection_pattern = r"^(\d+\.\d+)\s+([A-ZÁÉÍÓÚÑ][^:\n]+)$"
    cleaned_content = re.sub(
        subsection_pattern, r"### \1 \2", cleaned_content, flags=re.MULTILINE
    )

    # 4. Non-numbered section headers
    section_headers = [
        "RESUMEN EJECUTIVO",
        "ANÁLISIS TEMPORAL",
        "ANÁLISIS ESTACIONAL",
    ...
    for header in section_headers:
        pattern = f"^{header}$"
        replacement = header.title().replace("De ", "de ").replace("Y ", "y ")
        cleaned_content = re.sub(
            pattern, f"## {replacement}", cleaned_content, flags=re.MULTILINE
        )

    # 5. Convert bullet points (•) to proper markdown list items
    cleaned_content = re.sub(r"^•\s+", r"- ", cleaned_content, flags=re.MULTILINE)

    # FIX: Handle numbered list items in "Implicaciones Gerenciales" section
    ...
    if implications_match:
        implications_text = implications_match.group(1)
        # Convert numbered items to list format
        protected_implications = re.sub(
            r"^(\d+)\.\s+([^#\n]+?)$",
            r"- \2",
    ...
    # FIX: Handle the first metadata paragraph (should be smaller font)
    lines = cleaned_content.split("\n")
    if lines and len(lines) > 1:
        first_line = lines[0]
        if any(
            word in first_line
            for word in ["Fuentes:", "Período:", "Unidad:", "Metodología:", "Software:"]
        ) or ("Análisis de Tendencias" in first_line and len(first_line) > 100):
            lines[0] = f"<small>{first_line}</small>"
            ...
            if len(lines) > 1:
                ...
                if (
                    ...
                    or len(second_line) > 80
                ):
                    lines[1] = f"<small>{second_line}</small>"

            cleaned_content = "\n".join(lines)

    # Fix table formatting and remove References section for consistency
    lines = cleaned_content.split("\n")
    fixed_lines = []
    i = 0
    ...
    while i < len(lines):
        ...
        if (
            (
                "GT" in line
                and any(metric in line for metric in ["GB", "CR", "BUI", "BCSI"])
            )
            or ("CT" in line and "SixSig" in line)
            or ("Período" in line and "frecuencia" in line)
        ):
            # Collect table lines
            table_lines = []
            j = i
            ...
            while j < len(lines):
                ...
                is_table_line = current_line and (
                    re.search(r"\d+[.,]\d+", current_line)
                    or re.search(r"\d{2,}", current_line)
                    or current_line.count("|") >= 2
                    or any(
                    ...
                    or (
                        j == i
                        and (
                            "Período" in current_line
                            or "Metodología" in current_line
                            or "Software" in current_line
                        )
                    )
                )
            ...
            if table_lines:
                ...
                is_correlation_matrix = any(
                    "GT" in line and "GB" in line for line in table_lines[:3]
                )

                if is_correlation_matrix:
                    # Special handling for correlation matrices
                    # Extract the data in the format: header_row + data_rows
                    ...
                    for line in table_lines:
                        ...
                        # Check if this looks like a header row (starts with metric names)
                    ...
                    if header_row and data_rows:
                        # Create proper correlation matrix table
                        metrics = header_row[:5]  # First 5 metrics as column headers

                        # Header row
                        table_header = "|     | " + " | ".join(metrics) + " |"
                        formatted_table.append(table_header)
                        ...
                        # Data rows - each row starts with a metric name
                        for row_parts in data_rows:
                            if len(row_parts) >= 2 and row_parts[0] in metrics:
                                row_metric = row_parts[0]
                                values = row_parts[1 : len(metrics)]

                                # Pad values to match number of metrics
                                while len(values) < len(metrics):
                                    values.append("")

                                # Create row with empty diagonal (when row_metric == column_metric)
                                table_row = [""] * len(metrics)
                                for i, metric in enumerate(metrics):
                                    if row_metric != metric and i < len(values):
                                        table_row[i] = values[i]
                        ...
                        for k, table_line in enumerate(table_lines):
                            if "|" in table_line:
                                cleaned_line = re.sub(r"\s*\|\s*", " | ", table_line)
                                cleaned_line = re.sub(r"\s+", " ", cleaned_line)
                                formatted_table.append(cleaned_line)
                            else:
                                parts = re.split(r"\s{2,}", table_line)
                                parts = [p.strip() for p in parts if p.strip()]
                                if len(parts) >= 2:
                                    if k == 0:
                                        table_row = "| " + " | ".join(parts) + " |"
                ...
                fixed_lines.extend(formatted_table)
                fixed_lines.append("")
    ...
    # Remove References section for consistency (handle various formats)
    cleaned_content = re.sub(
        r"(?:\n\n)?(?:## )?Referencias.*", "", cleaned_content, flags=re.DOTALL
    )
    ...
    # 2. Multi-digit numbered main sections: "10. Something"
    multi_digit_main_pattern = r"^(\d{2,})\.\s+([A-ZÁÉÍÓÚÑ][^:\n]+)$"
    cleaned_content = re.sub(
        multi_digit_main_pattern, r"## \1. \2", cleaned_content, flags=re.MULTILINE
    )

    # 3. Sub-sections with decimal notation: "2.1 Tendencia", "3.2 Análisis"
    subsection_pattern = r"^(\d+\.\d+)\s+([A-ZÁÉÍÓÚÑ][^:\n]+)$"
    cleaned_content = re.sub(
        subsection_pattern, r"### \1 \2", cleaned_content, flags=re.MULTILINE
    )

    # 2. Non-numbered section headers - direct pattern matching
    direct_replacements = [
        # Exact matches with proper capitalization
        (r"^RESUMEN EJECUTIVO$", r"## Resumen Ejecutivo"),
        (r"^ANÁLISIS TEMPORAL$", r"## Análisis Temporal"),
        (r"^ANÁLISIS ESTACIONAL$", r"## Análisis Estacional"),
        (r"^ANÁLISIS ESPECTRAL$", r"## Análisis Espectral"),
        (r"^ANÁLISIS DE CALOR$", r"## Análisis de Calor"),
        (
            r"^ANÁLISIS DE COMPONENTES PRINCIPALES$",
            r"## Análisis de Componentes Principales",
        ),
        (r"^MATRIZ DE CORRELACIÓN$", r"## Matriz de Correlación"),
        (
            r"^IMPLICACIONES GERENCIALES$",
            r"## Implicaciones Gerenciales",
        ),  # Fixed pattern
        (r"^IMPLICACIONES ESTRATÉGICAS$", r"## Implicaciones Estratégicas"),
        (r"^LIMITACIONES$", r"## Limitaciones"),
        (r"^REFERENCIAS.*$", r"## Referencias"),
        (
            r"^CONCLUSIONES CLAVE PARA DIRECTIVOS$",
            r"## Conclusiones Clave para Directivos",
        ),
        # Lowercase variants
        (r"^resumen ejecutivo$", r"## Resumen Ejecutivo"),
        ...
        # Title case variants (first letter capitalized)
        (r"^Resumen Ejecutivo$", r"## Resumen Ejecutivo"),
        (r"^Análisis Temporal$", r"## Análisis Temporal"),
        (r"^Análisis Estacional$", r"## Análisis Estacional"),
        (r"^Análisis Espectral$", r"## Análisis Espectral"),
    ...
    for pattern, replacement in direct_replacements:
        cleaned_content = re.sub(
            pattern, replacement, cleaned_content, flags=re.MULTILINE
        )

    # 3. Sub-headers with dot notation like "2.1 Tendencia de largo plazo"
    subheader_pattern = r"^(\d+\.\d+)\s+([A-ZÁÉÍÓÚÑ][^:\n]+)$"
    cleaned_content = re.sub(
        subheader_pattern, r"### \1 \2", cleaned_content, flags=re.MULTILINE
    )

    # 4. Convert bullet points (•) to proper markdown list items
    cleaned_content = re.sub(r"^•\s+", r"- ", cleaned_content, flags=re.MULTILINE)

    # FIX 2: Handle numbered list items within sections (not section headers)
    # Patterns like "1. Text here" that should be list items, not headers
    # Look for these patterns AFTER section headers to avoid conflicts

    # First, protect numbered items that appear after section headers
    protected_patterns = []

    # Find "Implicaciones Gerenciales" section and protect its numbered items
    ...
    if implications_section:
        ...
        # Convert numbered items in this section to list format
        protected_implications = re.sub(
            r"^(\d+)\.\s+([^#\n]+?)$",
            r"- \2",  # Convert "1. Text" to "- Text"
            implications_text.strip(),
            flags=re.MULTILINE,
        )

        # Replace the section with protected version
        cleaned_content = cleaned_content.replace(
            implications_section.group(0),
            f"## Implicaciones Gerenciales\n\n{protected_implications}",
        )
    ...
    # FIX 3: Handle the first metadata paragraph (should be smaller font)
    # This is typically the introductory paragraph with sources, period, etc.
    lines = cleaned_content.split("\n")
    if lines and len(lines) > 1:
        # Check if first line contains metadata (sources, period, frequency)
        first_line = lines[0]
        if any(
            word in first_line
            for word in ["Fuentes:", "Período:", "Unidad:", "Metodología:", "Software:"]
        ) or ("Análisis de Tendencias" in first_line and len(first_line) > 100):
            # This looks like metadata - convert to smaller text
            lines[0] = f"<small>{first_line}</small>"

            # Check second line too if it seems like metadata continuation
            if len(lines) > 1:
                second_line = lines[1]
                if (
                    any(
    ...
    # Fix table formatting issues - look for specific table patterns
    lines = cleaned_content.split("\n")
    fixed_lines = []
    i = 0
    ...
    while i < len(lines):
        ...
        # Detect correlation matrix pattern (names + numbers in columns)
        if (
            (
                "GT" in line
                and any(metric in line for metric in ["GB", "CR", "BUI", "BCSI"])
            ...
            # This looks like a table - collect table rows
            table_lines = []
            j = i

            # Collect consecutive lines that could be table data
            while j < len(lines):
                current_line = lines[j].strip()

                # Check if line looks like table data (contains numbers or table indicators)
                is_table_line = (
                    current_line
                    and (
                        re.search(r"\d+[.,]\d+", current_line)  # Contains decimals
                        or re.search(
                            r"\d{2,}", current_line
                        )  # Contains multi-digit numbers
                        or current_line.count("|") >= 2  # Already has pipe separators
                        or any(
                            word in current_line
                            for word in [
                                "GT",
            ...
            if table_lines:
                ...
                for k, table_line in enumerate(table_lines):
                    # Clean and format table line
                    if "|" in table_line:
                        # Already has pipes - clean up spacing
                        cleaned_line = re.sub(r"\s*\|\s*", " | ", table_line)
                        cleaned_line = re.sub(r"\s+", " ", cleaned_line)
                        formatted_table.append(cleaned_line)
                    else:
                        # No pipes - split by multiple spaces and create table
                        parts = re.split(r"\s{2,}", table_line)  # Split by 2+ spaces
                        parts = [p.strip() for p in parts if p.strip()]

                        if len(parts) >= 2:
                            if k == 0:
                                # First row - add separator after
                                table_row = "| " + " | ".join(parts) + " |"
                                formatted_table.append(table_row)
                                # Add separator row
                                separator = (
                            ...
                            # Not enough parts for table - keep as regular text
                            formatted_table.append(table_line)

        # FIX 4: Fix correlation matrix table formatting - it's being transposed
        # The correlation matrix should have proper row headers and column headers
        if (
            "Matriz de Correlación" in cleaned_content or "Pearson" in cleaned_content
        ) and table_lines:
            # Special handling for correlation matrices
            formatted_table = []
            ...
            for k, table_line in enumerate(table_lines):
                if "|" in table_line:
                    # Already has pipes - this is likely the correctly formatted version
                    cleaned_line = re.sub(r"\s*\|\s*", " | ", table_line)
                    cleaned_line = re.sub(r"\s+", " ", cleaned_line)
                    formatted_table.append(cleaned_line)
                else:
                    # No pipes - this might be the transposed format, let's fix it
                    parts = re.split(r"\s{2,}", table_line)  # Split by 2+ spaces
                    parts = [p.strip() for p in parts if p.strip()]
                    ...
                    if len(parts) >= 2:
                        if k == 0:
                            ...
                            # Data rows - but check if this looks like a correlation matrix row
                            # If it starts with a metric name, it might be a row header
                            if parts[0] in [
                                "GT",
                                "GB",
                                "CR",
                                ...
                                # Might be continuation data - add as new row
                                table_row = (
                                    "| " + " | ".join(["..."] + parts[1:]) + " |"
                                )
                                formatted_table.append(table_row)
        ...
        # Add formatted table to fixed lines
        fixed_lines.extend(formatted_table)
        fixed_lines.append("")  # Empty line after table

    i = j  # Skip processed lines

    # FIX 5: Remove References section for consistency across all entries
    # Since not all database entries have this section, remove it to avoid inconsistency
    cleaned_content = "\n".join(fixed_lines)
    cleaned_content = re.sub(
        r"\n\n## Referencias.*", "", cleaned_content, flags=re.DOTALL
    )
...
def get_key_findings_with_database_first(
    tool_name, selected_sources, language="es", force_refresh=False
):
    """
    Generate Key Findings using database-first strategy (Phase 4).

    This function:
    1. Checks precomputed database first (sub-2ms for 1,302 cached analyses)
    2. Falls back to live AI only if not found in cache
    3. Provides instant results for common queries
    """
    global database_first_service, key_findings_service

    # Initialize database-first service if not already done
    if database_first_service is None and key_findings_service:
        try:
            # Import the database-first service
            import sys
            ...
            if database_first_service:
                print(
                    "✅ Database-first service initialized - instant cached results enabled"
                )
            else:
                print("⚠️ Database-first service not available - using live AI")

        except Exception as e:
            print(f"⚠️ Could not initialize database-first service: {e}")
            database_first_service = None

    # Use database-first service if available
    if database_first_service and not force_refresh:
        try:
            print(f"🔍 DATABASE-FIRST: Checking cache for {tool_name}")
            result = database_first_service.get_analysis_from_database(
                tool_name, selected_sources, language
            )
            if result:
                print(
                    f"✅ INSTANT RESULT: Found in database ({result['response_time_ms']}ms)"
                )
                return result
            print("❌ Not in cache, falling back to live AI")
        except Exception as e:
            print(f"⚠️ Database-first check failed: {e}")

    # Fall back to live AI if database-first not available or cache miss
    if key_findings_service:
        try:
            print(f"🔄 LIVE AI: Generating fresh analysis for {tool_name}")
            # This will use the existing live AI workflow
            ...
            print(f"❌ Live AI generation failed: {e}")
            return {
                "success": False,
                "error": f"Both database-first and live AI failed: {e}",
                "response_time_ms": 0,
                "source": "error",
            }
    ...
    return {
        "success": False,
        "error": "No Key Findings service available",
        "response_time_ms": 0,
        "source": "error",
    }
...
# Add API route for saving analysis to database
@server.route("/api/save-analysis-to-database", methods=["POST"])
def save_analysis_to_database():
    """
    API endpoint to save current analysis content to database.
    """
    try:
        import json
        ...
        print(f"💾 Saving analysis to database: {len(content)} characters")
        print(f"📅 Timestamp: {timestamp}")

        # Here you would implement the actual database save logic
        # For now, just return success
        return json.dumps({"success": True, "message": "Analysis saved successfully"})
...
# Hidden menu functionality removed to fix JavaScript errors
            ...
            [
                ...
                dbc.ModalFooter(
                    [
                        ...
                        # Regenerar button removed - use hidden admin menu (Ctrl+Shift+R then Ctrl+Shift+G)
    ...
    function handleRegenerateButtonClick(n_clicks) {
        if (n_clicks && n_clicks > 0) {
            ...
            if (button && spinnerContainer && buttonText) {
                ...
                console.log('🔄 CLIENTSIDE: Button state reset - regenerate removed');
            }
        }
        return window.dash_clientside.no_update;
    }
    """,
    # regenerate-key-findings callback removed - now use hidden admin menu only
)
...
if KEY_FINDINGS_AVAILABLE and key_findings_service:
    ...
    @app.callback(
        ...
        State("data-sources-store-v2", "data"),
        State("language-store", "data"),
        State("key-findings-modal", "is_open"),
        prevent_initial_call=True,
    )
    def regenerate_key_findings_analysis(
        regenerate_clicks,
        selected_tool,
        selected_sources,
        language,
        ...
        """Handle regeneration of Key Findings with force_refresh=True"""
        ctx = dash.callback_context

        if not ctx.triggered or not regenerate_clicks:
            return dash.no_update

        if trigger_id := ctx.triggered[0]["prop_id"].split(".")[0]:
            if trigger_id == "regenerate-key-findings" and modal_is_open:
                print("🔄 REGENERATION: Force regenerating analysis...")

                try:
                    # Force refresh - bypass database cache
                    key_findings_result = get_key_findings_with_database_first(
                        tool_name=selected_tool,
                        selected_sources=selected_sources,
                        language=language,
                        force_refresh=True,  # THIS IS THE KEY - force regeneration
                    )

                    if key_findings_result.get("success", False):
                        # Process the result and return new content
                        result_data = key_findings_result.get("data", {})
                        response_time_ms = key_findings_result.get(
                            "response_time_ms", 0
                        )

                        print(f"✅ Regeneration completed in {response_time_ms}ms")

                        # Return the regenerated content (you'll need to implement this rendering)
                        return html.Div(
                            [
                                html.Div(
                                    [
                                        ...
                                        html.P(
                                            f"Generado en {response_time_ms}ms con nueva interpretación",
                                            className="text-muted",
                                        ),
                                    ]
                                ),
                                # Add the actual regenerated content rendering here
                                # This should use the same rendering logic as the main modal
                            ]
                        )
                    else:
                        return html.Div(
                    ...
                    return html.Div(
                        [
                            html.H5(
                                "❌ Error en Regeneración", className="text-danger mb-3"
                            ),
    ...
    def toggle_key_findings_modal(
        ...
        if trigger_id == "generate-key-findings-btn":
            ...
            try:
                print(
                    "🚀 Starting Key Findings generation with DATABASE-FIRST strategy..."
                )
                print(
                ...
                # PHASE 4: DATABASE-FIRST STRATEGY - Use our new function
                print("🔄 Using database-first approach for instant results...")
                ...
                # Handle the result from database-first approach
                if not key_findings_result.get("success", False):
                    print(
                        f"❌ Database-first approach failed: {key_findings_result.get('error', 'Unknown error')}"
                    )
                    return (
                        True,
                        html.Div(
                            [
                                html.H4("Error", className="text-danger"),
                                html.P(
                                    f"Error generating analysis: {key_findings_result.get('error', 'Unknown error')}",
                                    className="text-muted",
                                ),
                            ]
                        ),
                ...
                # Extract data from database-first result
                result_data = key_findings_result.get("data", {})
                response_time_ms = key_findings_result.get("response_time_ms", 0)
                cache_hit = key_findings_result.get("cache_hit", False)
                source = key_findings_result.get("source", "unknown")

                print(f"✅ Analysis completed in {response_time_ms}ms from {source}")
                print(f"📊 Cache hit: {cache_hit}")

                # Create the content display based on the result
                content_parts = []
                    ...
                    content_parts.append(
                        html.Div(
                            [
                                ...
                                html.Span(
                                    f"Instant result from database ({response_time_ms}ms)",
                    ...
                    content_parts.append(
                        html.Div(
                            [
                                ...
                                html.Span(
                                    f"Generated fresh analysis ({response_time_ms}ms)",
                ...
                # Handle content based on database structure - use executive_summary for complete analysis
                executive_summary = result_data.get("executive_summary", "")
                if executive_summary:
                    # Complete analysis is stored in executive_summary field
                    cleaned_content = clean_content_headers(
                    ...
                    # Handle separate sections (legacy support)
                    content_sections = [
                        ("executive_summary", "", ""),
                        ("principal_findings", "", ""),
                        ("heatmap_analysis", "", ""),
                ...
                if result_data.get("confidence_score"):
                    metadata_parts.append(
                        f"Confianza: {result_data.get('confidence_score', 0):.2f}"
                    )
                if result_data.get("model_used"):
                    metadata_parts.append(f"Modelo: {result_data.get('model_used')}")
                if result_data.get("data_points_analyzed"):
                    metadata_parts.append(
                        f"Puntos de datos: {result_data.get('data_points_analyzed')}"
                    )
                if result_data.get("sources_count"):
                    metadata_parts.append(
                        f"Fuentes: {result_data.get('sources_count')}"
                    )

                if metadata_parts:
                    content_parts.append(html.Hr())
                    content_parts.append(
                        html.Div(
                            [
                                html.Small(
                                    " | ".join(metadata_parts), className="text-muted"
                                )
                            ],
                ...
                final_content = html.Div(content_parts)
                ...
                print(f"❌ Unexpected error in key findings generation: {e}")
                import traceback
                ...
                return (
                    ...
                    html.Div(
                        [
                            html.H4("Error Inesperado", className="text-danger"),
