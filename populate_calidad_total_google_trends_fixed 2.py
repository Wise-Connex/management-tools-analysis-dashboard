...
def populate_calidad_total_google_trends():
    ...
    print("🎯 Populating database with complete 7-section structure for Calidad Total + Google Trends...")
    print("=" * 80)
...
Las oportunidades de ventana temporal se presentan durante los períodos de planificación empresarial anual. La estrategia de implementación debe basarse en los ciclos identificados, aprovechando los momentos de mayor interés y preparación organizacional.""",
        ...
        if existing_record:
            ...
            print(f"   🔄 Updating existing record for {test_data['tool_name']} + {test_data['sources_text']}")
            cursor.execute("""
                ...
                SET executive_summary = ?, principal_findings = ?, temporal_analysis = ?,
                    ...
                    analysis_type = ?, computation_timestamp = ?
                WHERE id = ?
            """, (
                test_data["executive_summary"], test_data["principal_findings"], test_data["temporal_analysis"],
                test_data["seasonal_analysis"], test_data["fourier_analysis"], test_data["strategic_synthesis"],
                test_data["conclusions"], test_data["pca_analysis"], test_data["heatmap_analysis"],
                test_data["confidence_score"], test_data["model_used"], test_data["data_points_analyzed"],
                test_data["analysis_type"], datetime.now(), existing_record[0]
            ))
        else:
            # Insert new record
            print(f"   💾 Inserting new record for {test_data['tool_name']} + {test_data['sources_text']}")
            cursor.execute("""
                ...
                (tool_name, sources_text, language, executive_summary, principal_findings, 
                 ...
                 data_points_analyzed, analysis_type, is_active, computation_timestamp)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?)
            """, (
                test_data["tool_name"], test_data["sources_text"], test_data["language"], 
                test_data["executive_summary"], test_data["principal_findings"],
                test_data["temporal_analysis"], test_data["seasonal_analysis"], 
                test_data["fourier_analysis"], test_data["strategic_synthesis"],
                test_data["conclusions"], test_data["pca_analysis"], test_data["heatmap_analysis"],
                test_data["confidence_score"], test_data["model_used"], test_data["data_points_analyzed"],
                test_data["analysis_type"], datetime.now()
            ))
